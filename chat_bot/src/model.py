...
from v1.config import (block_size, dropout, embed_size, end_token, n_heads, n_layer)
...
class Head(nn.Module):
    def __init__(self, head_size):
        """
        This module performs self-attention operations on the input tensor,
        producing an output tensor with the same time-steps but different channels.
        
        Args:
            head_size (int): The size of the head in the multi-head attention.
        """
        super().__init__()
    ...
    def forward(self, x):
        ...
        wei = wei / math.sqrt(k.shape[-1])  # (B, T, T)
        
        # avoid look-ahead
        tril = torch.tril(torch.ones(T, T, device='cuda'))
...
class MultiHeadAttention(nn.Module):
    """
    This class contains multiple `Head` objects, which perform
    self-attention operations in parallel.
    """
    def __init__(self):
        super().__init__()
        
        # list of parallel heads that are concatenated
        # by the linear layer in the end
...
class Block(nn.Module):
    """
    This module contains a single transformer block, which consists of
    multi-head self-attention followed by feed-forward neural networks.
    """
    def __init__(self):
        super().__init__()
...
class GPTLanguageModel(nn.Module):
    """
    This class encompasses the entire GPT model, including the token
    and position embeddings, multiple transformer blocks, and output layer.
    """
    def __init__(self, vocab_size: int):
        ...
        # put one block after the other sequentially
        # (not parallel like multi-head attention)
    ...
    def generate(self, idx, vocab):
        ...
        idx_next = torch.zeros(1, device='cuda')
        idx_end = encode([end_token], vocab).to('cuda')
        # idx_unk = encode([unknown_token], vocab).to('cuda')
        idx_sptk = encode(['blad'], vocab).to('cuda')
